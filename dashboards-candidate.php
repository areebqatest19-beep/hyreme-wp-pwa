...
<body>
    <div class="container">
        <div class="sidebar">
            ...
            <nav onclick="switchSection(event)" style="flex: 1;">
                ...
                <div class="nav-item" data-section="messages"><span>💬</span> My Messages</div>
            </nav>
        ...
        <div class="main-content">
                ...
                <div class="card">
                    ...
                    <div>
                        ...
                        <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 0.5rem; padding: 1rem;">
                            <video style="width: 100%; height: 200px; object-fit: contain;" controls><source src="<?php echo esc_url($intro_video_url); ?>" type="video/mp4"></video>
                            <button onclick="deleteVideoAJAX('intro')" style="color: #fca5a5; margin-top: 10px;">🗑️ Delete Video</button>
                        </div>
                        <?php else: ?>
                        <div class="upload-zone" data-video-type="intro">
                            <p>📹 Drag and drop intro video</p>
            ...
            <div id="analytics" class="section">
                <div class="card">
                    <h2>📊 Analytics</h2>
                    <div class="analytics">
                        <div class="stat-card"><div class="number"><?php echo intval($views_count); ?></div><div class="label">Views</div></div>
                        <div class="stat-card"><div class="number"><?php echo intval($saved_by_recruiters); ?></div><div class="label">Saved</div></div>
                        <div class="stat-card"><div class="number"><?php echo intval($messages_count); ?></div><div class="label">Messages</div></div>
                    </div>
                </div>
            </div>
            ...
            <div id="messages" class="section">
                <div class="card">
                    <h2>💬 Inbox</h2>
                    <div id="candidateChatList" class="space-y-4">
                         <p class="text-gray-400">Your recruiter chats will appear here...</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function uploadVideoAJAX(file, videoType) {
            ...
            previewContainer.innerHTML = `<video src="${videoUrl}" controls style="width:100%; height:200px;"></video>
                                          <button onclick="confirmUpload('${videoType}')">Confirm Upload</button>`;
            window['tempFile_' + videoType] = file;
        }

        function confirmUpload(videoType) {
            ...
            .then(r => r.json()).then(d => { if(d.success) location.reload(); else alert(d.data); });
        }

        function deleteVideoAJAX(type) {
            fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({ action: 'hyreme_delete_video', nonce: document.getElementById('hyreme_video_nonce').value, video_type: type })
